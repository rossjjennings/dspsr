  ...
  dsp::InverseFilterbank* filterbank)
{
  ...
  // setup scratch space
  int input_fft_points = n_per_sample*input_fft_length;
  int output_fft_points = 2*output_fft_length; // always return complex result
  int response_stitch_points = n_per_sample*input_fft_length*input_nchan;
  ...
  output_fft_scratch = input_fft_scratch + output_fft_points;
  response_stitch_scratch = output_fft_scratch + response_stitch_points;
  fft_shift_scratch = response_stitch_scratch + fft_shift_points;
  stitch_scratch = fft_shift_scratch + stitch_points;
}
              ...
              uint64_t npart, uint64_t in_step, uint64_t out_step)
{
  static const int floats_per_complex = 2;
	...
	const uint64_t n_pol = in->get_npol();
		...
		for(uint64_t ipart = 0; ipart < npart; ipart++) {
			for(unsigned ipol = 0; ipol < n_pol; ipol++) {
				for(unsigned input_ichan = 0; input_ichan < input_nchan; input_ichan++) {
					...
					cerr << "dsp::InverseFilterbankEngineCPU::perform: input_ichan=" << input_ichan << " getting freq_dom_ptr" << endl;
					freq_dom_ptr = input_fft_scratch;
					cerr << "dsp::InverseFilterbankEngineCPU::perform: input_ichan=" << input_ichan << " freq_dom_ptr=" << freq_dom_ptr << endl;
					cerr << "dsp::InverseFilterbankEngineCPU::perform: input_ichan=" << input_ichan << " getting time_dom_ptr" << endl;
					time_dom_ptr = const_cast<float*>(in->get_datptr(input_ichan, ipol));
					cerr << "dsp::InverseFilterbankEngineCPU::perform: input_ichan=" << input_ichan << " time_dom_ptr=" << time_dom_ptr << endl;
					time_dom_ptr += n_dims*ipart*(input_fft_length - input_discard_total);
					cerr << "dsp::InverseFilterbankEngineCPU::perform: input_ichan=" << input_ichan << " time_dom_ptr (after increment)=" << time_dom_ptr << endl;
